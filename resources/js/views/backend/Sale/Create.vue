<template>
    <div class="row sale-system">
                            ...
                            <div class="card-body p-0">
                                ...
                                <div class="card-text p-2">
                                    ...
                                    <div v-if="product.quantity > 0"
                                        ...
                                        <h6 class="price">Price: {{$root.settings.currency}} {{ product.sale_price
                                            }}</h6>
                                    </div>
                        ...
                        <div class="table-responsive">
                            <table
                                ...
                                <tr v-for="(cart,key) in $store.state.cart" :key="key">
                                    ...
                                    <td class="price">
                                        <span>{{$root.settings.currency}}{{ cart.sale_price.toFixed(2) }}</span>
                                    </td>
                                    <td class="discount">
                                        <span>{{$root.settings.currency}}{{ cart.discount_amount.toFixed(2) }}</span>
                                    </td>
                                    <td class="total">
                                        <span>{{$root.settings.currency}}{{ cart.total_price.toFixed(2) }}</span>
                                    </td>
                                    <td class="action">
                                ...
                                <tr>
                                    <td colspan="6" class="text-center py-5">
                                        <img height="100" :src="$root.baseurl+'/admin/images/empty.gif'" alt="">
                                        <h6>Empty Cart</h6>
                                    </td>
                                </tr>
                                </tbody>
        ...
        <b-modal size="xl" id="placeOrderSuccessfully" centered>
            <div class="row">
                <div class="col-lg-2 px-0">
                    <h4 class="mb-3">Select Printer</h4>
                    <button @click="printerMode = 'thermalprinter'"
                            class="btn btn-warning btn-border mb-3 d-block w-100" type="button">
                        <i class="fa fa-print"></i> Thermal Printer
                    </button>
                    <button @click="printerMode = 'a4printer'" class="btn btn-secondary btn-border  mb-3 d-block w-100"
                            type="button">
                        <i class="fa fa-print"></i> A4 Size Printer
                    </button>
                </div>
                <div class="col-lg-10">
                    <ThermalPrinter :invoice="invoice" v-if="printerMode == 'thermalprinter'"></ThermalPrinter>
                    <A4Printer :invoice="invoice" v-if="printerMode == 'a4printer'"></A4Printer>
                </div>
            </div>
        ...
        <div class="cart-preview">
            <div class="backdrop" @click="isConfirmOrder = false" v-show="isConfirmOrder"></div>
            <transition
                name="fade"
                enter-active-class="animate__animated animate__backInUp"
                leave-active-class="animate__animated animate__backOutDown">
                <div class="preview-box" v-show="isConfirmOrder">
                    <div class="card outbox">
                        <div class="card-header d-flex justify-content-between align-items-center">
                            <h4>Order Summary</h4>
                            <button @click="isConfirmOrder = false" type="button" class="btn-close"></button>
                        </div>
                        <div class="card-body">
                            <div class="row mb-3">
                                <div class="col-xl-8">
                                    <div class="row align-items-center gy-3 mb-3">
                                        <div class="col-sm">
                                            <div>
                                                <h5 class="fs-14 mb-0">Your Cart ({{ $store.state.cartCount }}
                                                    items)</h5>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="card product mb-1" v-for="(product,pindex) in $store.state.cart"
                                         :key="pindex">
                                        <div class="card-body">
                                            <div class="row gy-3">
                                                ...
                                                <div class="col-sm">
                                                    ...
                                                    <ul class="list-inline text-muted">
                                                        <li class="list-inline-item">
                                                            Quantity : <span
                                                            class="fw-medium">{{ product.quantity }}</span>
                                                        </li>
                                                    </ul>

                                                    <div class="input-step">
                                                        <button type="button" v-if="product.quantity > 1"
                                                                @click="decrement(product)" class="minus">–
                                                        </button>
                                                        <button type="button" v-else disabled="disabled" class="minus">
                                                            –
                                                        </button>
                                                        <input type="number" class="product-quantity"
                                                               :value="product.quantity" min="1">
                                                        <button type="button" @click="increment(product)" class="plus">
                                                            +
                                                        </button>
                                                    </div>
                                                </div>
                                                <div class="col-sm-auto">
                                                    <div class="text-lg-end">
                                                        ...
                                                        <h5 class="fs-14">{{$root.settings.currency}}<span
                                                            class="product-price">{{ product.sale_price.toFixed(2) }}</span>
                                                        </h5>
                                                    </div>
                                                    <div class="d-flex align-items-center gap-2 text-muted mt-4">
                                                        <div>Total :</div>
                                                        <h5 class="fs-14 mb-0">{{$root.settings.currency}}<span
                                                            class="product-line-price">{{ product.total_price.toFixed(2) }}</span>
                                                        </h5>
                                                    </div>
                                                </div>
                                            </div>
                                    ...
                                    <div class="sticky-side-div">
                                        <div class="card">
                                            ...
                                            <div class="card-header bg-soft-light border-bottom-dashed">
                                                <div class="">
                                                    <h5>Name : {{ shippingInfo.customer ? shippingInfo.customer.name :
                                                        ''}}</h5>
                                                    <div>
                                                        <span class="alpha-6"><b>Address:</b></span>
                                                        <span class="">{{ shippingInfo.address }}</span>
                                                    </div>
                                                    <div>
                                                        <span class="alpha-6"><b>Postal Code:</b></span>
                                                        <span class="">{{ shippingInfo.postal_code }}</span>
                                                    </div>
                                                    <div>
                                                        <span class="alpha-6"><b>City:</b></span>
                                                        <span class="" v-if="shippingInfo.country">{{ shippingInfo.city.name }}</span>
                                                    </div>
                                                    <div>
                                                        <span class="alpha-6"><b>State:</b></span>
                                                        <span class="" v-if="shippingInfo.state">{{ shippingInfo.state.name }}</span>
                                                    </div>
                                                    <div>
                                                        <span class="alpha-6"><b>Country:</b></span>
                                                        <span class=""
                                                              v-if="shippingInfo.country">{{ shippingInfo.country.name }}</span>
                                                    </div>
                                                    <div>
                                                        <span class="alpha-6"><b>Phone:</b></span>
                                                        <span class="">{{ shippingInfo.mobile }}</span>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="card-body pt-2">
                                                <div class="table-responsive">
                                                    <table class="table table-borderless mb-0">
                                                        ...
                                                        <tr>
                                                            <td>Total Discount <span class="text-muted"></span> :
                                                            </td>
                                                            <td class="text-end" id="cart-discount">
                                                                {{$root.settings.currency}} {{
                                                                parseInt(cart.product_discount) +
                                                                parseInt(cart.invoice_discount ? cart.invoice_discount :
                                                                0)}}
                                                            </td>
                                                        </tr>
                                                        <tr>
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td class="text-end" id="cart-tax">
                                                                {{$root.settings.currency}} {{ cart.vat + cart.tax
                                                                }}
                                                            </td>
                                                        </tr>
                                                        <tr>
                                                            <td>Total Recieve:</td>
                                                            <td class="text-end" id="cart-recieve">
                                                                {{$root.settings.currency}} {{
                                                                cart.total_recieve ? cart.total_recieve : 0 }}
                                                            </td>
                                                        </tr>
                                                        <tr class="table-active">
                                                            ...
                                                            <span class="fw-semibold" id="cart-total">
                                                                {{$root.settings.currency}}{{ cart.grand_total }}
                                                            </span>
                                                            </td>
                                                        </tr>
                                                        </tbody>
                                        ...
                                        <div class="row px-2">
                                            <template v-if="!smLoader.cod && !smLoader.card">
                                                <button @click="confirmOrderWithPay('cod')"
                                                        class="btn btn-secondary btn-border mb-2 d-block">
                                                    <i v-if="!smLoader.cod" class="fa fa-handshake-o"></i>
                                                    <i v-else class="fa fa-spinner fa-spin"></i>
                                                    Continue With COD
                                                </button>
                                            </template>
                                            <template v-else>
                                                <button disabled class="btn btn-secondary btn-border mb-2 d-block">
                                                    <i v-if="!smLoader.cod" class="fa fa-handshake-o"></i>
                                                    <i v-else class="fa fa-spinner fa-spin"></i>
                                                    Continue With COD
                                                </button>
                                            ...
                                            <template v-if="!smLoader.cod && !smLoader.card">
                                                <button @click="confirmOrderWithPay('card')"
                                                        class="btn btn-danger btn-border d-block">
                                                    <i v-if="!smLoader.card" class="fa fa-cc-mastercard"></i>
                                                    <i v-else class="fa fa-spinner fa-spin"></i>
                                                    Continue With Card
                                                </button>
                                            </template>
                                            <template v-else>
                                                <button disabled class="btn btn-danger btn-border mb-2 d-block">
                                                    <i v-if="!smLoader.card" class="fa fa-cc-mastercard"></i>
                                                    <i v-else class="fa fa-spinner fa-spin"></i>
                                                    Continue With Card
                                                </button>
...
<script>
    ...
    export default {
        components: {A4Printer, ThermalPrinter, PrintOption, VueSlickCarousel},
        data() {
            return {
                printerMode: 'a4printer',
        ...
        watch: {
            ...
            'cart.total_recieve'(value) {
                let total  = (parseInt(value) - this.cart.grand_total);
                this.cart.change = total > 0 ? total : 0;
            },
            ...
            'cart.shipping_id'(shippingId) {
                this.getCustomerShippingInfo(shippingId)
            },
            $route: {
                handler(value) {
                    if (value.query.invoice_id) {
                        this.getInvoice(value.query.invoice_id)
                    }
                },
                deep: true,
                immediate: true,
        ...
        methods: {
            ...
            search() {
                ...
                this.get_paginate_data('sale-product', this.search_data);
            },
            getProducts() {
            ...
            getCustomerShippingInfo(customerId) {
                axios.get('shipping-info/customer/' + customerId).then(res => {
            ...
            placeOrder() {
                ...
                if (!this.cart.shipping_id) {
                    this.notification("Please Add Shipping Address", "error");
                    return false
                }
                this.isConfirmOrder = true;
            },
            //*********** Confirm with payment method Handler  *************************
            confirmOrderWithPay(paywith) {
                this.cart['products'] = this.$store.state.cart;
                this.cart['total_quantity'] = this.$store.state.cartCount;
                this.cart['payment_method'] = paywith;
                this.$root.validation_errors = {};
                this.smLoader[paywith] = true
                axios.post(this.model, this.cart)
                    ...
                    .then(alw => setTimeout(() => (this.smLoader[paywith] = false), 400));
            },

            getInvoice(invoiceId) {
                axios.get('get-sales-invoice/' + invoiceId).then(res => {
        ...
        async created() {
            await this.get_paginate_data("sale-product", this.search_data);
            await this.setBreadcrumbs(this.model, "create");
